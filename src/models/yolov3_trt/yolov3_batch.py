...
class TrtYOLOv3(object):
    ...
    def __init__(self, model, input_shape=(416, 416), multithread=False):
        ...
        self.inference_fn = self.do_inference
    ...
    def do_inference_v2(self, context, bindings, inputs, outputs, stream, batch_size):
        ...
        if batch_size == 1:
            ...
            context.execute_async_v2(bindings=bindings, stream_handle=stream.handle, batch_size=batch_size)
        # Transfer predictions back from the GPU.
        [cuda.memcpy_dtoh_async(out.host, out.device, stream) for out in outputs]
        # Synchronize the stream
        stream.synchronize()
    ...
    def detect(self, imgs, conf_th=0.5, batch_size=1):
        """Detect objects in the input image."""
        pre_start_time = time.time()
        shape_orig_WH = (imgs[0].shape[1], imgs[0].shape[0])
        img_resized = []
        for i in range(batch_size):
            img_resized.append(_preprocess_yolov3(imgs[i], self.input_shape))
        ...
        self.inputs[0].host = np.ascontiguousarray(img_resized[0])
...
class Yolov3:
    ...
    def infer(self, img, batch_size):
        boxes, confs, classes, _, _,_ = self.trt_yolov3.detect(img, self.conf_th, batch_size)
    ...
    def warm_up(self, img_path='./utils/warm_up.jpg'):
