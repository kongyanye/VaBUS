...
class TrtYOLOv3(object):
    ...
    def _load_engine(self):
        TRTbin = 'models/yolov3_trt/yolov3_onnx/%s.trt' % self.model
    ...
    def __init__(self, model, input_shape=(416, 416), multithread=False):
...
class Yolov3:
    ...
    def __init__(self, conf_th=0.5, multithread=False, warm_up=False):
        self.name = 'Yolo_v3'
        self.conf_th = conf_th

        self.trt_yolov3 = TrtYOLOv3('yolov3-416', input_shape=(416, 416), multithread=multithread)
        self.relevant_class = Yolov3.reverse_relevant_class()
        self.coco_classes = {
    ...
    def reverse_relevant_class(cls):
        res = {}
        for k, v in Yolov3.relevant_class.items():
            for each in v:
                res[each] = k
        return res

    def infer(self, img):
        boxes, confs, classes = self.trt_yolov3.detect(img, self.conf_th)

        res = []

        for i in range(len(boxes)):
            if classes[i] in self.relevant_class:
                label = self.relevant_class[classes[i]]
            else:
                continue
            conf = confs[i]
            box = boxes[i]
    ...
    def infer_rpn(self, img, conf_th=0.1):
        boxes, confs, classes = self.trt_yolov3.detect(img, conf_th)
        ...
        for i in range(len(boxes)):
            ...
            box = boxes[i].tolist()
            if conf > 0.5:
                res.append([label, box[0], box[1], box[2], box[3]])
            else:
                rpn.append([label, box[0], box[1], box[2], box[3]])

        rpn = self.filter_rpn(res, rpn)
        return res, rpn

    def filter_rpn(self, res, rpn):
        # remove region > 0.04
        rpn1 = []
        for label, x1, y1, x2, y2 in rpn:
            if (x2-x1)*(y2-y1) > 720*406*0.04:
                continue
            rpn1.append([label, x1, y1, x2, y2])

        # remove 0.3 IoU with res
        rpn2 = []
        for label, x1, y1, x2, y2 in rpn1:
            box1 = (x1, y1, x2, y2)
            include = True
            for _, rx1, ry1, rx2, ry2 in res:
                box2 = (rx1, ry1, rx2, ry2)
                if iou(box1, box2) > 0.3:
                    include = False
                    break
            if include:
                rpn2.append([label, x1, y1, x2, y2])
        return rpn2
    ...
    def warm_up(self):
        """infer one image to warm up the model"""
        img = np.random.randint(0, 255, (406, 720), 'uint8')
        _ = self.infer(img)
