...
from .utils.caf_scored import CafScored
from .utils.cif_seeds import CifSeeds
from .utils.cif_hr import CifHr
from .utils.occupancy import Occupancy as UtilsOccupancy
from .. import headmeta
from ..visualizer.occupancy import Occupancy as VisualizerOccupancy
from ..visualizer.cif import Cif
from ..visualizer.caf import Caf
...
class CifCaf(Decoder):
    ...
    occupancy_visualizer = VisualizerOccupancy()
    ...
    def __init__(self,
        ...
        if self.cif_visualizers is None:
            self.cif_visualizers = [Cif(meta) for meta in cif_metas]
        self.caf_visualizers = caf_visualizers
        if self.caf_visualizers is None:
            self.caf_visualizers = [Caf(meta) for meta in caf_metas]

        if self.nms is True:
            self.nms = nms.Keypoints()
    ...
    def __call__(self, fields, initial_annotations=None):
        ...
        occupied = UtilsOccupancy(cifhr.accumulated.shape, 2, min_scale=4)
        annotations = []
    ...
    def complete_annotations(self, cifhr, fields, annotations):
        ...
        caf_scored = CafScored(cifhr.accumulated, score_th=0.0001).fill(
